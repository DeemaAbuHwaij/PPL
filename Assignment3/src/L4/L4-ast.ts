...
const parseBoundExp = (params: Sexp[]): Result<BoundExp> =>
    (params.length === 0) ? makeFailure(`bound? missing argument. ${params}`) :
    (params.length > 1) ? makeFailure(`bound? received too many arguments.`) :
    isSexpString(params[0]) ? makeOk(makeBoundExp(makeVarRef(params[0].toString()))):
    isString(params[0]) ? makeOk(makeBoundExp(makeVarRef(params[0]))):
    makeFailure(`not a valid var name`);
...
const unpraseBoundExp = (be: BoundExp): string =>
    `(bound? ${be.var.var})`;

const unpraseTimeExp = (be: TimeExp): string =>
    `(time ${unparse(be.exp)})`;
...
export const unparse = (exp: Parsed): string =>
    ...
    isBoundExp(exp) ? unpraseBoundExp(exp) :
    isTimeExp(exp) ? unpraseTimeExp(exp) :
