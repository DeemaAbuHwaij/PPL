...
import {
    ...
    isAppExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isProcExp, Binding, VarDecl, CExp, Exp, IfExp, LetrecExp, LetExp, ProcExp, Program, SetExp, parseL4Exp,
    DefineExp,isBoundExp,isTimeExp,BoundExp,unparse,TimeExp} from "./L4-ast";
import {
...
const evalBound = (exp: BoundExp, env: Env): Result<boolean> =>
    isGlobalEnv(env)? any((fb: FBinding) =>
        fb.var === exp.var.var, unbox(env.frame).fbindings)?makeOk(true) :
            makeOk(false): any((fb: FBinding) =>
                fb.var === exp.var.var, env.frame.fbindings)?makeOk(true) :
                    evalBound(exp, env.env);
