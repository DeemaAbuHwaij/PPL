...
*/
import { chain, concat, map, uniq, equals, sort, uniqWith } from "ramda";
import { Sexp } from "s-expression";
import { isEmpty, isNonEmptyList, second, List } from "../shared/list";
...
export const makeUnionTexp = (comp: TExp[]): UnionTExp => ({tag: "UnionTexp", components: comp});
...
const parseCompoundTExp = (texps: Sexp[]): Result<CompoundTExp> => {
    ...
    return !isNonEmptyList<Sexp>(texps) ? makeFailure(`empty compound expression`) :
        texps[0] === "union" ? parseUnionTexp(rest(texps)) :
        parseProcTexp(texps);
};


const parseUnionTexp = (texps: Sexp[]): Result<UnionTExp> => {
    const list_res: Result<List<TExp>> = parseListOfUnions(texps, []);
    const reduced_list = bind(list_res, list => makeOk(uniqWith(isequalTexp) (list)));
    const string_comp = (a: TExp, b: TExp) => a.tag === b.tag ? 0 : a.tag < b.tag ? -1 : 1;
    const sortedList_res = bind(reduced_list, (list) => makeOk(sort(string_comp ,list)));
    return bind(sortedList_res, (sortedlist) => makeOk(makeUnionTexp(sortedlist)));
}

const isequalTexp = (t1: TExp, t2: TExp): boolean =>
    isAtomicTExp(t1) && isAtomicTExp(t2) ? eqAtomicTExp(t1, t2) :
    isTVar(t1) && isTVar(t2) ? t1.var === t2.var :
    isProcTExp(t1) && isProcTExp(t2) ? equals(procTExpComponents(t1), procTExpComponents(t2)) :
    false; 

const parseListOfUnions = (texps: Sexp[], list: List<TExp>): Result<TExp[]> => {
    if (!isNonEmptyList<Sexp>(texps) || texps.length !== 2) {
        return makeFailure(`union has to be between 2 texp`);
    } 
    else {
        const first_te_res = parseTExp(first(texps));
        const second_te_res = parseTExp(second(texps));
        const first_te_list = bind(first_te_res, (te1: TExp): Result<TExp[]> => isUnionTexp(te1) ? makeOk(te1.components) : makeOk([te1])); 
        const second_te_list = bind(second_te_res, (te2: TExp): Result<TExp[]> => isUnionTexp(te2) ? makeOk(te2.components) : makeOk([te2])); 
        return safe2((te1_list: TExp[], te2_list: TExp[]) => makeOk([...te1_list, ...te2_list])) (first_te_list, second_te_list);
    }
}
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const unparseUnion = (components: TExp[]): Result<string> => 
        isNonEmptyList<TExp>(components) ?
            isNonEmptyList<TExp>(rest(components)) ? safe2((x: string | string[] , y: string | string[]) =>
                makeOk(`(union ${x} ${y})`)) (up(first(components)), unparseUnion(rest(components))) :
            bind(up(first(components)), x => makeOk(`${x}`)):
        makeOk(``);
