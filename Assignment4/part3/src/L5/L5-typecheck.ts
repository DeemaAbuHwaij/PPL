// L5-typecheck
// ========================================================
import { equals, filter, forEach, map, pipe, reduce, sort, union, zipWith } from 'ramda';
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         isPrimOp, isProcExp, isProgram, isStrExp, isVarRef, parseL5Exp, unparse,
         AppExp, BoolExp, DefineExp, Exp, IfExp, LetrecExp, LetExp, NumExp,
         Parsed, PrimOp, ProcExp, Program, StrExp, makeProgram, parseL5Program, isAtomicExp, makeStrExp } from "./L5-ast";
import { applyTEnv, isEmptyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isUnionTexp, makeUnionTexp, ProcTExp, isTupleTExp } from "./TExp";
import { isEmpty, allT, first, rest, NonEmptyList, List, isNonEmptyList, cons } from '../shared/list';
import { Result, makeFailure, bind, makeOk, zipWithResult, safe2, isOk, isFailure } from '../shared/result';
import { parse as p } from "../shared/parser";
import { format } from '../shared/format';
import { isatty } from 'tty';
import { isAtomicTExp } from './TExp';
import { isTVar } from './TExp';
import { type } from 'os';
...
export const checkCompatibleTypeWithUnions = (te1: TExp, te2: TExp, exp: Exp): Result<true> => {
    const typeset1: TExp[] = isUnionTexp(te1) ? te1.components : [te1];
    const typeset2: TExp[] = isUnionTexp(te2) ? te2.components : [te2];
    const ret_val: Result<true> = isContainedIn(typeset1, typeset2) ? makeOk(true) :
        bind(unparseTExp(te1), (te1: string) =>
        bind(unparseTExp(te2), (te2: string) =>
            bind(unparse(exp), (exp: string) => 
                makeFailure<true>(`Incompatible types: ${te1} and ${te2} in ${exp}`))));
...
// given 2 sets of types determines if the first set contained in the second set.
export const isContainedIn = (typeset1: TExp[], typeset2: TExp[]): boolean => 
    typeset1.length > typeset2.length ? false :
    filter( (texp1: TExp) => !belongsTo(texp1, typeset2), typeset1).length === 0;


// given type exp and set of type exps checks if the type belongs to the set.
export const belongsTo = (texp: TExp, typeset: TExp[]): boolean => 
    !isNonEmptyList<TExp>(typeset) ? false :
    isCompetibleTypes(texp, first(typeset)) ? true :
    belongsTo(texp, rest(typeset));
        
// check competibelity of 2 non union
export const isCompetibleTypes = (te1: TExp, te2: TExp): boolean => 
    te1.tag !== te2.tag ? false :
    isAtomicTExp(te1) && isAtomicTExp(te2) ? te1.tag === te2.tag :
    isTVar(te1) && isTVar(te2) ? te1.var === te2.var :
    isProcTExp(te1) && isProcTExp(te2) ? isOk(checkComptibleProcstypes(te1, te2)) :
    false;

export const checkComptibleProcstypes = (procTE1: ProcTExp, procTE2: ProcTExp): Result<true> => 
    procTE1.paramTEs.length !== procTE2.paramTEs.length ? makeFailure("") :
    bind(checkCompatibleTypeWithUnions(procTE1.returnTE, procTE2.returnTE, makeStrExp("")), 
            pipe(
                _ => zipWith( (t1: TExp, t2: TExp) => checkCompatibleTypeWithUnions(t2, t1, makeStrExp("")), procTE1.paramTEs, procTE2.paramTEs),
                (a: Result<true>[]) => reduce((acc, elem) => isFailure(acc) || isFailure(elem) ? elem : makeOk(true) , makeOk(true), a)
            )
        );
...
export const L5typeofprogram = (program: string): Result<string> =>
    bind(p(program), (x) =>
        bind(parseL5Program(x), (e: Parsed) => 
            bind(typeofExp(e, makeEmptyTEnv()), unparseTExp)));
...
export const makeUnion = (te1: TExp, te2: TExp): TExp => {
    const te1_comp = isUnionTexp(te1) ? te1.components : [te1];
    const te2_comp = isUnionTexp(te2) ? te2.components : [te2];
    const _union = union(te1_comp, te2_comp);
    const sorted_union = sort((a: TExp, b:TExp) => a.tag === b.tag ? 0 : a.tag < b.tag ? -1 : 1, _union);
    const ret_val: TExp = isNonEmptyList<TExp>(sorted_union) && sorted_union.length === 1 ? first(sorted_union) : makeUnionTexp(sorted_union);
    return ret_val; 
}
...
export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> => {
    {
        ...
        const bodyTE = safe2( (t1: TExp, t2: TExp) => makeOk(makeUnion(t1, t2))) (thenTE, altTE);
        return bind(constraint1, _ => bodyTE);
    };
};
...
//      type<val>(tenv) = t2
//      t2 ⊆ t1
// THEN type<(define var val)>(tenv) = void
// TODO L51 - write the true definition
export const typeofDefine = (exp: DefineExp, tenv: TEnv): Result<VoidTExp> =>
    bind(bind(typeofExp(exp.val, tenv),
        (typeOfVal) => checkCompatibleTypeWithUnions(exp.var.texp, typeOfVal, exp)), _ => makeOk(makeVoidTExp()));
...
export const typeofProgram = (exp: Program, tenv: TEnv): Result<TExp> =>
    ! isNonEmptyList<Exp>(exp.exps) ? makeOk(makeVoidTExp()) :
    isDefineExp(first(exp.exps)) ? typeofProgram(makeProgram(rest(exp.exps)), extendGlobalTenv(first(exp.exps), tenv)) :
    typeofExps(exp.exps, tenv);

export const extendGlobalTenv = (exp: Exp, tenv: TEnv): TEnv => 
    isDefineExp(exp) ?
    isEmptyTEnv(tenv) ? makeExtendTEnv([exp.var.var], [exp.var.texp], makeEmptyTEnv()) :
    makeExtendTEnv(cons(exp.var.var, tenv.vars), cons(exp.var.texp, tenv.texps), makeEmptyTEnv()) :
    makeEmptyTEnv();